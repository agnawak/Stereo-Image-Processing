...
pathL = "./images/Stereo/Calibration/L/"
pathR = "./images/Stereo/Calibration/R/"
...
for i in tqdm(range(1, 7)): # change based on number of pictures taken for the calibration
    ...
    retL, cornersL = cv2.findChessboardCorners(imgL_gray, chessboard_size, None)
    retR, cornersR = cv2.findChessboardCorners(imgR_gray, chessboard_size, None)
    ...
    if retR and retL:
        ...
        cv2.drawChessboardCorners(outputL, chessboard_size, cornersL2, retL)
        cv2.drawChessboardCorners(outputR, chessboard_size, cornersR2, retR)
...
cv_file = cv2.FileStorage("./MonoParam6.xml", cv2.FILE_STORAGE_WRITE)
...
out[:, :, 1] = Right_nice[:, :, 1]
out[:, :, 2] = Left_nice[:, :, 2]
cv2.imwrite("./images/Stereo/Calibration/rectifiedL.png",Left_nice)
cv2.imwrite("./images/Stereo/Calibration/rectifiedR.png",Right_nice)
cv2.imwrite("./images/Stereo/Calibration/output.png", out)
